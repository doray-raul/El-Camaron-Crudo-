...
@login_required(login_url='/crm/login/')
def interacciones_view(request, cliente_id=None):
    interacciones = Interaccion.objects.select_related(
        'cliente',
        'usuario',
    ).order_by('-fecha')
    ...
    cliente_id = cliente_id or request.GET.get('cliente')
    cliente = None
    if cliente_id:
        cliente = get_object_or_404(Cliente, id=cliente_id)
        interacciones = interacciones.filter(cliente=cliente)

    return render(request, 'crm/interacciones.html', {
        'interacciones': interacciones,
...
@login_required(login_url='/crm/login/')
def nueva_interaccion_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('interacciones_cliente', cliente_id=interaccion.cliente_id)
    else:
        form = InteraccionForm(initial={'cliente': cliente})

    return render(request, 'crm/nueva_interaccion.html', {
        'form': form,
...
def mi_actividad_view(request):
    # Cada usuario consulta exclusivamente las interacciones que registró.
    # El campo ``usuario`` se asigna al crear la interacción, por lo que el
    # historial se conserva al cerrar sesión y volver a entrar con esa cuenta.
    interacciones = Interaccion.objects.filter(
...
def editar_usuario_view(request, usuario_id):
    ...
    # Un administrador no puede modificar otra cuenta administrativa. Solo el
    # superusuario puede hacerlo, para evitar que un administrador cambie el
    # rol, contraseña o datos de otro administrador.
    if usuario.is_staff and usuario != request.user and not request.user.is_superuser:
        messages.error(
            request,
            'Solo el superusuario puede modificar a otro administrador.'
        )
        return redirect('usuarios')
...
def eliminar_usuario_view(request, usuario_id):
    """Solo el superusuario puede eliminar cuentas del CRM."""
